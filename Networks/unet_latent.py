...
class UNet_Latent(BaseNetwork):
    def __init__(self, n_steps, name, time_emb_dim, in_channels, hidden_channels, 
        ...
        for i in range(num_layers):
            if i == 0:
                ...
                te_layer = self._make_te(time_emb_dim, 120)
            else:
        ...
        for i in reversed(range(num_layers)):
            ...
            te_layer = self._make_te(time_emb_dim, in_channels*2)    
            ...
            if i != 0:
                ...
                for j in range(block_layers):
                    if j == 0:
                        decoder_block.append(ConvBlock((in_channels * 2, 8, 8), in_channels * 2, out_channels, act))
                    elif j == 1:
        ...
        self.conv_out = torch.nn.Conv2d(out_channels, 120, 3, 1, 1)
    ...
    def noise_prediction(self, ins, t):
        ...
        for i, (decoder_block, te_layer) in enumerate(zip(self.decoder_blocks, self.te_up_list)):
            out = torch.cat((down_outputs[-idx], self.ups[i](out)), dim=1)
            out = decoder_block(out + te_layer(t).reshape(n, -1, 1, 1))
            idx = idx + 1
...
class Conditional_UNet_Latent(BaseNetwork):
    def __init__(self, n_steps, name, time_emb_dim, label_emb_dim, num_classes, 
        ...
        for i in range(num_layers):
            if i == 0:
                out_channels = hidden_channels
                te_layer = self._make_te(time_emb_dim, 120)
                lb_layer = self._make_lb(label_emb_dim, 120)
            else:
                te_layer = self._make_te(time_emb_dim, in_channels)
        ...
        for i in reversed(range(num_layers)):
            ...
            te_layer = self._make_te(time_emb_dim, in_channels*2) 
            lb_layer = self._make_lb(label_emb_dim, in_channels*2)
            up = torch.nn.Sequential(torch.nn.ConvTranspose2d(in_channels, in_channels, 3, 1, 1),
                                     act())
    ...
    def noise_prediction(self, ins, t, lb):
        ...
        for i, (decoder_block, te_layer, lb_layer) in enumerate(zip(self.decoder_blocks, self.te_up_list, self.lb_up_list)):
            out = torch.cat((down_outputs[-idx], self.ups[i](out)), dim=1)
            out = decoder_block(out + te_layer(t).reshape(n, -1, 1, 1) + lb_layer(lb).reshape(n, -1, 1, 1))
            idx = idx + 1
