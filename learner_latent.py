...
class Learner_Latent(BaseDifussionLearning):
    ...
    def _train_epoch(self, train=True):
        ...
        for i, data in enumerate(self.train_data):
            ...
            noise_pred_de, noise_pred_latent = self.noise_prediction(noisy_images_latent, 
                                                            time_steps.reshape(len(images), -1))
            ...
            #         print(f"Gradients for {name}: mean={param.grad.mean()}, max={param.grad.max()}, min={param.grad.min()}")            
            ...
            if train:
                ...
                #         fake_images_de = generate_images_latent(self.ddpm, self.device, noisy_images_latent, self.learner_config["denoising_option"])
                ...
                #         self.fake_images_initial = fake_images_de.detach()
                #         self.generated_images_initial = generated_images_de
                #         self.noise_tensors_latent_initial = noise_tensor_latent
                #         self.noises_latent_initial = noisy_images_latent
    ...
    def _hook_every_epoch(self):
        ...
        #     self.noise = torch.randn(30, self.latent_dm.in_channels_de, 4, 4).to(self.device)
        
        # gen_imgs_de_fixed, noise_latent_fixed = generate_new_images_latent(self.latent_dm, n_samples=30, 
        #                                         device=self.device,
        ...
        # gen_imgs_de, noise_latent = generate_new_images_latent(self.latent_dm, n_samples=30, 
        #                                         device=self.device,
        #                                         channels=self.latent_dm.in_channels_de, 
        #                                         height=4, width=4,
        #                                         denoising_option=self.learner_config["denoising_option"])
...
class Conditional_Learner_Latent(Learner_Latent):
    ...
    def _hook_every_epoch(self):
        ...
        #                                         fixed_noise=self.noise,
        #                                         denoising_option=self.learner_config["denoising_option"],
        #                                         labels=self.test_labels.reshape(30, -1))
