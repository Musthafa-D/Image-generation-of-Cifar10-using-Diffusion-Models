...
from Networks.trained_encoder_decoder import Encoder_Decoder, Conditional_Encoder_Decoder
...
def generate_images_latent(latent_dm, device, noisy_images_latent, denoising_option, labels=None):
    """
    Given a DDPM model, a number of samples (like batch size)
    to be generated and a device, returns some newly generated samples
    ...
    with torch.no_grad():
        ...
        n_samples = x.size(0)
        for idx, t in enumerate(list(range(latent_dm.n_steps))[::-1]):
            time_tensor = (torch.ones(n_samples, 1) * t).to(device).long()
            
            if labels is not None:
                _, noise_pred = latent_dm.noise_prediction_labels(x, time_tensor, labels)             
            else:
                _, noise_pred = latent_dm.noise_prediction(x, time_tensor)
            
            alpha_t = latent_dm.alphas[t]
            alpha_t_bar = latent_dm.alpha_bars[t]
            ...
            if t > 0:
                ...
                if denoising_option == 1:
                    ...
                    # Option 2: sigma_t squared = beta_tilda_t
                    prev_alpha_t_bar = latent_dm.alpha_bars[t-1] if t > 0 else latent_dm.alphas[0]
                    beta_tilda_t = ((1 - prev_alpha_t_bar)/(1 - alpha_t_bar)) * beta_t
                    sigma_t = beta_tilda_t.sqrt()
                ...
                x = x + sigma_t * z
        if labels is not None:
            x_de = latent_dm.conditional_decoder(x, labels)
        else:
            x_de = latent_dm.decoder(x)
    return x_de

def generate_new_images_latent(latent_dm, n_samples, device, channels, height, width, denoising_option, fixed_noise=None, labels=None):
    """
    Given a DDPM model, a number of samples (like batch size)
    to be generated and a device, returns some newly generated samples
...
def load_en_de():
    """
    Loads the pre trained encoder and decoder
    ...
    en_de_config = config["trained_en_de"]  
    en_de = Encoder_Decoder(**en_de_config).to(DEVICE)
    checkpoint_path = os.path.join("Saved_networks", "en_de_net_best.pt")
    
    checkpoint = torch.load(checkpoint_path)
    en_de.load_state_dict(checkpoint['model_state_dict'])
    en_de.eval()
    return en_de

def load_c_en_de():
    """
    Loads the pre trained conditional encoder and decoder
    ...
    c_en_de_config = config["trained_conditional_en_de"]  
    c_en_de = Conditional_Encoder_Decoder(**c_en_de_config).to(DEVICE)
    checkpoint_path = os.path.join("Saved_networks", "c_en_de_net_best.pt")
    
    checkpoint = torch.load(checkpoint_path)
    c_en_de.load_state_dict(checkpoint['model_state_dict'])
    c_en_de.eval()
    return c_en_de
